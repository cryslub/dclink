			...
			<div class="col inspection-tab" style="padding-left: 20px;">
				...
				<div class="row">
					...
					<div class="col-lg-6">
						...
						<div style="margin-bottom: 5px;">
							<small class="text-muted" ng-if="(items | toArray:false  |  filter : {'type':'피폭'}).length > 0">주목할 만한 시전과 피폭</small>
							<small class="text-muted" ng-if="(items | toArray:false  |  filter : {'type':'장면'}).length > 0">장면</small>
						...
						<ul class="list-group"
							...
							<li class="list-group-item ">
								...
								<div
									ng-repeat="item in items | toArray:false  | filter : {'type':'장면'}">
									<hr ng-if="!$first">
									<small><span
										class="glyphicon glyphicon-alert text-danger"
										style="margin-right: 5px;"></span> </small> {{item.name}} <a
							...
							<li class="list-group-item ">
								<div>
									...
									<span class="glyphicon glyphicon-tower "
										style="color: gold; margin-right: 5px;"></span> {{item.name}} <a
										ng-href="{{item.link}}" ng-if="item.link!=''" target="youtube"
										style="margin-right: 10px;"> <small> 듣기</small></a> <span><small>
					...
					<div class="modal-body" style="padding-left: 10px;">
						...
						<div ng-repeat="(person,item) in history ">
							...
							<div ng-if="persons[person].photo == true">
								...
								tooltip-animation="false" uib-tooltip="꾸준 출마자 "></span>
								<span class="glyphicon glyphicon-bishop" style="top: 34px;position: relative;color: gold;" ng-if="objectMax(item.inspectionCount)>=20"
								tooltip-animation="false" uib-tooltip="프로 국감러"></span>
								<span class="glyphicon glyphicon-bishop" style="top: 34px;position: relative;color: silver;" ng-if="objectMax(item.inspectionCount)>=13 && objectMax(item.inspectionCount)<=19"
								tooltip-animation="false" uib-tooltip="프로 국감러"></span>
							...
							<div ng-if="persons[person].photo != true" style="padding-left:16px;">
								...
								ng-if="(item.election | filter:positiveRate).length==3"
								tooltip-animation="false" uib-tooltip="꾸준 출마자 "></span>
								<span class="glyphicon glyphicon-bishop" style="color: gold;" ng-if="objectMax(item.inspectionCount)>=20"
								tooltip-animation="false" uib-tooltip="프로 국감러"></span>
								<span class="glyphicon glyphicon-bishop" style="color: silver;" ng-if="objectMax(item.inspectionCount)>=13 && objectMax(item.inspectionCount)<=19"
								tooltip-animation="false" uib-tooltip="프로 국감러"></span>
							...
							<div ng-show="item.inspection.length>0"
								...
								<span ng-repeat="i in item.inspection" class="inspection"
									...
									title="<strong>{{i.txt==''?i.name:i.txt}}</strong>"
									ng-init="setPopover('inspection'+$index)"
									data-placement="bottom" ng-mouseenter="i.hover = true"
									ng-mouseleave="i.hover = false"> 
										<span ng-if="i.type=='이슈'"
										...
										<span
										ng-if="i.type=='피폭' || i.type=='장면'" class="glyphicon  item"
										ng-class="{'glyphicon-alert':!i.hover,'glyphicon-headphones':i.hover}"
										ng-style="{'color':parties[i.party].color}"></span> 
										<span
										ng-if="i.type=='파이날'&&i.txt!='MVP'" class="glyphicon  item"
										ng-class="{'glyphicon-tower':!i.hover,'glyphicon-headphones':i.hover}"
										ng-style="{'color':parties[i.party].color}"></span>
										<span
										ng-if="i.txt=='MVP'" class="glyphicon  item"
										ng-class="{'glyphicon-king':!i.hover,'glyphicon-headphones':i.hover}"
										ng-style="{'color':parties[i.party].color}"></span>
				...
				<div class="modal-content">
					<div class="modal-header">
						<h5 class="modal-title">{{zoneTitle}} 
							<small  style="margin-left:3px;color:silver;top: 0px; position: relative;"> 
							...
							<div ng-repeat="item in eh" style="margin-bottom: 2px;"
								...
								<h6 ng-class="{'no-margin':result=='show'}">
									...
									<small ng-repeat="candidate in item.candidates"
										...
										target="youtube" style="text-decoration: none;"> <span class="glyphicon glyphicon-user"
...
//    	    			$scope.drawRate('result'+index,item.candidates,5);
	...
	$scope.makeInspection = function(arr){
    	arr.forEach(function(item) {
    		...
    		if($scope.history[item.person].inspectionCount[item.election] == undefined){
    			$scope.history[item.person].inspectionCount[item.election] =0;
    		}
    		var add = 1;
    		if(item.type=='이슈'){
    			add = 2;
    		...
    		$scope.history[item.person].inspectionCount[item.election]+= add;
    	});
